...
const StudentTabs = ({ activeTab, setActiveTab, showCommunity = false, isActiveStudent = true, onMoreClick }) => {
  const baseTabs = [
  ...
  return (
    <nav className="student-tabs">
      {isActiveStudent && baseTabs.map(renderTab)}
      {showCommunity && renderTab(hittingPartnersTab)}
      {renderTab(moreTab)}
    </nav>
  )
}
